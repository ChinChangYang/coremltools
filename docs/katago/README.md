...
See [BACKEND_INTEGRATION.md](BACKEND_INTEGRATION.md) for detailed documentation on integrating the converted Core ML model as a backend in KataGo.
...
- Integrated into existing KataGo GTP workflow

**Requirements:**
- KataGo source code modifications
- Objective-C++ bridge for Core ML APIs
- Backend registration and configuration

**Implementation status**: Future work (not yet implemented)

For implementation details, architecture, and code examples, see the full backend integration guide.
...
| Channels | Blocks | File Size | Inference (Apple Silicon) | Strength |
|----------|--------|-----------|--------------------------|----------|
| 128      | 10     | ~50MB     | ~20-30ms                | Amateur  |
| 256      | 20     | ~150MB    | ~40-60ms                | Strong Amateur |
| 384      | 28     | ~250MB    | ~60-80ms                | Dan Level |
| 512      | 28     | ~300MB    | ~80-100ms               | Professional |
| 512      | 40     | ~400MB    | ~120-150ms              | Top Professional |
